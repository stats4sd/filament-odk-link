...
> **Remediation status (branch `pre-publish-fixes`, updated 2026-07-23):** B1–B11, B14, B16, B17 and B18 fixed (B9, B10, B16 and B18 as uncommitted working-tree changes on the branch). B12, B13 and B15 remain open. Verified with the full Pest suite (174 passed) and PHPStan, no new errors. Per-finding status is noted inline against each item below.
...
### B10 ✅ `updateUser()` / `deleteUser()` declare `: array` but return nothing → `TypeError`
...
**Status — ✅ fixed (branch `pre-publish-fixes`):** removed both methods; they were stubs; unused and un-expanded code.
...
**Status — ⚠️ still open (verified 2026-09-08, tracked in [#132](https://github.com/stats4sd/filament-odk-link/issues/132)):** the `str_split` coordinate handling is unchanged in `UpdateXlsformTitleInFile.php`.
...
**Status — ⚠️ still open (verified 2026-09-08, tracked in [#133](https://github.com/stats4sd/filament-odk-link/issues/133)):** the `end repeat` branch still pops the stack unguarded.
...
**Status — ⚠️ still open (verified 2026-09-08, tracked in [#134](https://github.com/stats4sd/filament-odk-link/issues/134)):** `ProcessOdkSubmission::dispatch()` still precedes `getAttachedMedia()` in both `getSubmissions()` and `getOneSubmission()`.

### B16 ✅ Broken Entity/DatasetVariable relations
...
**Status — ✅ fixed (branch `pre-publish-fixes`):** `Entity::datasetVariables()` now passes `relatedKey: 'name'` so the join compares `dataset_variables.name = entity_values.dataset_variable_name`, and `DatasetVariable::values()` is now `hasMany(EntityValue::class, 'dataset_variable_name', 'name')`. Writing tests exposed a further break: the relation's `->using(EntityValue::class)` fatals on hydration (`Call to undefined method EntityValue::fromRawAttributes()`) because `EntityValue` extends plain `Model` — it now uses the `AsPivot` trait. Covered by [EntityDatasetRelationsTest.php](../../tests/Unit/Models/EntityDatasetRelationsTest.php).

### B17 ✅ `Locale::owners()` selects a misspelled pivot column → SQL error
...
**Status — ✅ fixed (branch `pre-publish-fixes`, commit `d455a33`):** the pivot column is now `language_id` and the `belongsToMany` casing was corrected in the same commit.

### B18 ✅ `Entity::addValues()` de-dupe compounds the suffix
...
**Status — ✅ fixed (branch `pre-publish-fixes`):** the loop now captures `$baseName` before mutation and builds each candidate as `"{$baseName}.{$count}"`, yielding `field`, `field.1`, `field.2`, … with a 500-iteration guard against runaway loops. Covered by [EntityDatasetRelationsTest.php](../../tests/Unit/Models/EntityDatasetRelationsTest.php).

## Lower / latent (null-safety & robustness)

_Re-verified against `dev` on 2026-09-08. Open items are tracked as GitHub issues (linked); two have since been fixed._

- [#135](https://github.com/stats4sd/filament-odk-link/issues/135) **Unguarded `owner->odkProject->id`** in QR/link generation ([HasXlsformDrafts.php:106](../../src/Models/OdkLink/Abstracts/HasXlsformDrafts.php#L106), [Xlsform.php:248](../../src/Models/OdkLink/Xlsform.php#L248), [XlsformTemplate.php:389](../../src/Models/OdkLink/XlsformTemplate.php#L389), [Submission.php:186](../../src/Models/OdkLink/Submission.php#L186)) — fatals when an owner has no ODK project (the exact scenario a recent hotfix acknowledged).
- [#136](https://github.com/stats4sd/filament-odk-link/issues/136) **`makeMultiSelectBooleans` null choice-list deref** ([OdkSubmissionService.php:544](../../src/Services/OdkLinkServices/OdkSubmissionService.php#L544)) — no guard when the parsed `select_multiple` list name isn't in the loaded `$choices`.
- [#137](https://github.com/stats4sd/filament-odk-link/issues/137) **N+1 choice-list query inside a repeat-group loop** ([OdkSubmissionService.php:437](../../src/Services/OdkLinkServices/OdkSubmissionService.php#L437)); N+1 version lookup per submission ([:163](../../src/Services/OdkLinkServices/OdkSubmissionService.php#L163)).
- [#138](https://github.com/stats4sd/filament-odk-link/issues/138) **Export N+1**: `getLanguageStrings()` calls the relation as a query builder, bypassing eager loading — ~`6·L·R` queries per survey export ([ExportsXlsformContent.php:36](../../src/Exports/XlsformExport/ExportsXlsformContent.php#L36)).
- [#139](https://github.com/stats4sd/filament-odk-link/issues/139) / [#140](https://github.com/stats4sd/filament-odk-link/issues/140) / [#141](https://github.com/stats4sd/filament-odk-link/issues/141) **Import null-derefs on malformed workbooks**: unmatched module in [GetsModuleNamesPerRow.php:24](../../src/Imports/XlsformTemplate/GetsModuleNamesPerRow.php#L24); unknown choice list in [XlsformTemplateSurveyImport.php:61](../../src/Imports/XlsformTemplate/XlsformTemplateSurveyImport.php#L61); `ChoiceListModelsExport::headings()` on an empty list ([ChoiceListModelsExport.php:72](../../src/Exports/ChoiceListModelsExport.php#L72)).
- [#142](https://github.com/stats4sd/filament-odk-link/issues/142) **`SurveyExport` null-safety lie** — `?Xlsform $xlsform = null` constructor immediately dereferences it ([SurveyExport.php:17](../../src/Exports/SurveyExport.php#L17)).
- [#143](https://github.com/stats4sd/filament-odk-link/issues/143) **`DatasetAsMediaAttachmentExport` emits `name`/`label` twice** (intended exclusions commented out, [DatasetAsMediaAttachmentExport.php:66](../../src/Exports/DatasetAsMediaAttachmentExport.php#L66)) — corrupts the CSV ODK reads for `select_*_from_file`.
- ✅ _fixed — `forgetToken()` added to `OdkLinkService` and a 401 retry in `OdkFormService`._ **Token never invalidated**: `authenticate()` caches for 20h with no `Cache::forget` / retry-on-401 anywhere ([OdkLinkService.php:38](../../src/Services/OdkLinkService.php#L38)); an early server-side session expiry wedges the whole integration until TTL elapses.
- [#144](https://github.com/stats4sd/filament-odk-link/issues/144) **Hardcoded panel route** `route('filament.admin.resources.datasets.view', …)` ([DatasetTable.php:30](../../src/Filament/OdkAdmin/Resources/Datasets/Tables/DatasetTable.php#L30)) → `RouteNotFoundException` on any panel not named `admin`; use `DatasetResource::getUrl(...)`.
- ✅ _fixed with B5._ **`helperText($property['helper_text'])`** references a key the repeater never defines (keys are `name`/`label`/`hint`) ([ChoiceListEntriesRelationManager.php:47](../../src/Filament/OdkAdmin/Resources/ChoiceListResource/RelationManagers/ChoiceListEntriesRelationManager.php#L47)).
- [#145](https://github.com/stats4sd/filament-odk-link/issues/145) **Broken anchor markup** (missing closing quote on `href`) in `custom-odk-templates-widget.blade.php:5`.
- [#146](https://github.com/stats4sd/filament-odk-link/issues/146) **`# of Active Xlsforms` counts all xlsforms** (`->counts('xlsforms')`, not active) in the relation-manager tables.
- [#147](https://github.com/stats4sd/filament-odk-link/issues/147) **`createProjectAppUser` display name** relies on `+` binding tighter than `.` — works on 8.4 but fragile ([OdkProjectService.php:52](../../src/Services/OdkLinkServices/OdkProjectService.php#L52)).
- [#148](https://github.com/stats4sd/filament-odk-link/issues/148) **`XlsformValidationHelper` only accepts `_en`** while the accepted/valid header format is `label::English (en)` — flags valid forms as errors ([XlsformValidationHelper.php:119](../../src/Services/XlsformValidationHelper.php#L119)).
...
2. **Data integrity:** B3, B4, B7, B8, B9, B16, B17 — wrong data or hard errors on used paths. — _All ✅ done (B5 from the table above also ✅ done)._
3. **Robustness:** the null-deref / N+1 cluster — B12/B13/B15 and the lower/latent list, tracked in [#132](https://github.com/stats4sd/filament-odk-link/issues/132)–[#148](https://github.com/stats4sd/filament-odk-link/issues/148). Token invalidation is done.
