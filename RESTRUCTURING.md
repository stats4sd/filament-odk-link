...
**Status**: In Progress (steps 2–4 complete, step 5 next, see Roadmap) **Branch**: one branch per step off `dev` (`restructure` for step 2, `reference-app` for step 3) **Started**: 2026-09-08 **Origin**: [docs/code-reviews/2026-07-06-architecture-package-vs-app.md](docs/code-reviews/2026-07-06-architecture-package-vs-app.md) and its companion [docs/code-reviews/2026-07-06-package-review.md](docs/code-reviews/2026-07-06-package-review.md). Documentation was added to version control in PR #153; implementation records now accompany the tracked roadmap.
...
├── docs/                         tracked plans, change-logs and code-reviews
...
- [x] **2. Mechanical move into `packages/odk-link-core`.** Merged as PR #149.
  ...
  - [x] Add a root `CLAUDE.md` pointing agents at the package-level `packages/odk-link-core/CLAUDE.md` and this file, and repair the `@.claude/...` includes in the package `CLAUDE.md` (the `.claude/` directory stayed at root). The untracked root `AGENTS.md` from before the move is gone. **Correction (step 3):** this was ticked in PR #149 but the root `CLAUDE.md` was never committed and the package includes were still broken. Both landed in the step 3 PR.
  - [x] Commit, PR to `dev`.
- [x] **3. Add `apps/reference`.** Fresh Laravel 13 + Filament 5 install with `Team` and `User`, an `admin` panel loading `OdkLinkAdmin` and a tenant `team` panel, requiring core by path. Published package config and migrations committed, seeded roles/user/team/platform, 17 smoke tests, `reference-ci.yml`. Plan: [docs/plans/2026-09-08-reference-app.md](docs/plans/2026-09-08-reference-app.md) (local only). Details in [apps/reference/README.md](apps/reference/README.md).
- [x] **4. Purge host-app coupling behind contracts.** Implemented and locally verified on `restructuring-prep-for-ui-split`; PR review/merge remains pending. Canonical `FormOwner`/`PlatformUser`, injectable submission processing and notification recipients, validated current-owner and notification-delivery seams, explicit model registry, independent core registration and Filament adapters replace the actual host/UI coupling. Reference models/config demonstrate these contracts. Architecture tests cover shipping source, migrations, config and routes. Core tests: 230 passed; reference: 22 passed, one existing skip; PHPStan, both Pint checks and Composer validation pass. Step 5 is next. Plan: [docs/plans/2026-09-18-restructuring-step-4-host-contracts.md](docs/plans/2026-09-18-restructuring-step-4-host-contracts.md).
- [ ] **5. Extract `packages/odk-link-filament`.** `git mv` `src/Filament` (including the integration provider and adapters), `src/Forms`, UI testing helpers, `resources/views`, `resources/dist`, the plugin entry point and the asset build config. Drop `filament/filament` from core's `composer.json`. PHPStan on core without Filament installed is the enforcer.
...
- Root `.gitignore` patterns for `vendor` and `node_modules` are unanchored, so they cover nested package directories. `docs/` became tracked in PR #153. `composer.lock` is no longer ignored anywhere.
...
- Step 5 onward: when a second package gets a test suite, add a `filament-ci.yml` by copying `core-ci.yml` and changing `defaults.run.working-directory` and `env.PKG`. **Prefix the job names** (`reference-ci.yml` uses `reference tests (PHP 8.4)`, `reference pint`): required status checks match by check-run name across all workflows, so a second job called `pint` would be indistinguishable from the required `core-ci` one and could block merges. The earlier note here claiming check names are per-workflow was wrong.
- `reference-ci` is not in the `dev` ruleset yet. Add `reference tests (PHP 8.4)` once it has run green a few times. It installs from the committed `apps/reference/composer.lock`; Composer does not re-resolve a path package's `require` on `install`, so a change to the core's `composer.json` must be followed by `composer update stats4sd/filament-odk-link` in the app and a lock commit, or reference CI tests the old transitive set.
- The reference app holds **published copies** of the package config and migrations (`apps/reference/config/filament-odk-link.php`, `apps/reference/database/migrations/2026_09_08_*`). The package never calls `runsMigrations()`, so publishing is the only way a host gets the schema. When the core adds a migration, re-publish in the app and commit; re-publishing keeps existing timestamps and appends new files, so a migration that must run mid-sequence needs a manual rename. Procedure in the app README.
- Package issues surfaced by the reference app, deferred beyond Step 4 (list with detail in `apps/reference/README.md`, "Known package issues"): storage config reads the misspelled key `filesystem.default`; `PlatformSeeder` skips the `Platform` row the admin resource needs when `ODK_URL` is empty; template view and edit pages read `rootSection->schema` and a `bool` closure returns `null`, so they cannot render for a template that has not been imported; saving a template without a file calls ODK Central through `getRequiredMedia()`; `HasXlsforms::country()` reads `owner_id` on the owner table; the submission update route has no middleware; three migrations (`016`, `018`, `027`) pass on SQLite but fail on MySQL (unindexed FK target, `nulllable()` typo, `default(0)` on an FK column). One package edit was made in step 3: the `XlsformTemplateInfolist` import in `XlsformTemplateResource` was case-corrected to `XlsformTemplateInfoList` because it fataled on Linux.
- There is no `OdkLinkTeam` plugin in the package; the package `CLAUDE.md` claimed one. The reference app's `team` panel therefore has tenancy wired but nothing from the package in it until steps 5 and 6.
- `.env` files cannot be read or written by the agent tooling in this repo (permission deny rule). The app README documents the `.env` lines a developer adds by hand; `.env.example` is the untouched skeleton copy.
