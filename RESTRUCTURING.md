...
- `reference-ci` is not in the `dev` ruleset yet. Add `reference tests (PHP 8.4)` once it has run green a few times.
...
- Package issues surfaced by the reference app, parked for step 4 (list with detail in `apps/reference/README.md`, "Known package issues"): storage config reads the misspelled key `filesystem.default`; `PlatformSeeder` skips the `Platform` row the admin resource needs when `ODK_URL` is empty; template view and edit pages call ODK Central and need an imported xlsx, so they cannot render in local-only mode; `HasXlsforms::country()` reads `owner_id` on the owner table; the submission update route has no middleware. One package edit was made in step 3: the `XlsformTemplateInfolist` import in `XlsformTemplateResource` was case-corrected to `XlsformTemplateInfoList` because it fataled on Linux.
