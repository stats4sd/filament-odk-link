...
**Status**: In Progress (step 3 of 7 complete, step 4 next, see Roadmap) **Branch**: one branch per step off `dev` (`restructure` for step 2, `reference-app` for step 3) **Started**: 2026-09-08 **Origin**: [docs/code-reviews/2026-07-06-architecture-package-vs-app.md](docs/code-reviews/2026-07-06-architecture-package-vs-app.md) and its companion [docs/code-reviews/2026-07-06-package-review.md](docs/code-reviews/2026-07-06-package-review.md). Note that `docs/` is gitignored at the root, so those files exist only on the machine where the reviews were run.
...
- [ ] **4. Purge host-app coupling behind contracts.** Replace `App\Models\*` in `Dataset`, `ImportAllLanguageStrings`, the two `NotifyUserThat*` jobs and `HelperService`. Replace the `ClassFinder` scan and `Role::findByName`. Define `FormOwner`, `PlatformUser`, `SubmissionProcessor`, `RoleResolver` contracts bound via config. Add arch tests forbidding `App\` and `Filament\` imports in core. Plan: [docs/plans/2026-09-18-restructuring-step-4-host-contracts.md](docs/plans/2026-09-18-restructuring-step-4-host-contracts.md) (local only; includes the corrected current coupling inventory).
