...
- [ ] **2. Mechanical move into `packages/odk-link-core`.** In progress.
  ...
  - [ ] Fix `.github/workflows/*`. They still reference root `composer.json` and `vendor/bin/pest`. They are also stale independent of the move: `run-tests.yml` targets branch `main`, PHP 8.1/8.2, Laravel 10, while the package targets PHP 8.4, Laravel 13 and merges to `dev`. Rewrite rather than patch. Plan: [docs/plans/2026-09-08-github-workflows-monorepo.md](docs/plans/2026-09-08-github-workflows-monorepo.md) (local only, `docs/` is gitignored).
  - [ ] Add a root `CLAUDE.md` pointing agents at the package-level `packages/odk-link-core/CLAUDE.md` and this file, and repair the `@.claude/...` includes in the package `CLAUDE.md` (the `.claude/` directory stayed at root). The untracked root `AGENTS.md` from before the move is gone.
  - [ ] Commit, PR to `dev`.
...
- The move happened on branch `restructure`. Nothing is committed yet as of this writing.
